<template>
  <q-page v-if="auto" padding>
      ...
      <q-responsive class="col-12 col-md-6" :ratio="16/9">
        ...
        <q-carousel-slide v-for="[index,foto] in auto.imagine_auto.entries()" :key="foto.id" class="column no-wrap flex-center" :name="(index+1)">
          <q-img fit="contain" no-spinner :src="foto.foto">
            ...
            <div style="padding:8px;">{{(index+1)}}/{{auto.imagine_auto.length}}</div>
          </q-img>
    ...
    <q-card v-if="auto.descriere && !auto.vandut" class="my-card col-12 col-md-6 q-mt-md">
      <q-card-section class="text-h6">Descriere</q-card-section>
      <q-card-section> 
      <div style="white-space: pre-wrap;">{{auto.descriere}}</div>
      </q-card-section>
    </q-card>
